...
func TestSoleEnabledNodeTransportErrorSkipsHardCooldown(t *testing.T) {
	...
	manager := NewManager(repository, cipher)
	// Warm the node snapshot so sole detection does not need a re-entrant List.
	if _, err := manager.listNodes(context.Background(), domain.ScopeWeb, time.Now().UTC()); err != nil {
		t.Fatal(err)
	}
...
func TestFixedProxyTransportFailureStillCreatesCooldown(t *testing.T) {
	...
	// With multiple fixed peers, a transport failure must cool the failed peer
	// so healthy siblings are preferred. Sole single-node setups skip hard
	// cool-down and are covered by TestSoleEnabledNodeTransportErrorSkipsHardCooldown.
	repository := &mutableMultiEgressRepository{nodes: []domain.Node{
		{ID: 1, Name: "fixed-a", Scope: domain.ScopeBuild, Enabled: true, Health: 1},
		{ID: 2, Name: "fixed-b", Scope: domain.ScopeBuild, Enabled: true, Health: 1},
	}}
	manager := NewManager(repository, cipher)
	if _, err := manager.listNodes(context.Background(), domain.ScopeBuild, time.Now().UTC()); err != nil {
		t.Fatal(err)
	...
	if node.FailureCount != 1 || node.CooldownUntil == nil || node.LastError != "transport error" {
		t.Fatalf("fixed transport failure did not create cooldown: %#v", node)
	}
}
