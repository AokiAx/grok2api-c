...
func (m *Manager) acquire(ctx context.Context, scope domain.Scope, affinity string, allowDirect bool, encryptedCredentialCookies string, boundNodeID uint64) (*Lease, bool, error) {
	...
	if boundNodeID != 0 {
		...
		// Sole single-exit fixed nodes stay usable during cool-down; multi-peer
		// fixed nodes still honor cool-down so healthy siblings are preferred.
		if !proxyPool && selected.CooldownUntil != nil && now.Before(*selected.CooldownUntil) {
			if _, err := m.listNodes(ctx, selected.Scope, now); err != nil {
				return nil, true, err
			}
			if !m.isSoleEnabledNode(ctx, selected) {
	...
	for _, candidateScope := range fallbackScopes(scope) {
		...
		for _, node := range nodes {
			...
			configured = true
			// Shared proxy pools must stay selectable even if a request-level
			// failure left a stale global cool-down mark on the shared node.
			if m.isProxyPoolNode(node) {
				node.Health, node.FailureCount, node.CooldownUntil, node.LastError = 1, 0, nil, ""
				candidateAvailable = append(candidateAvailable, node)
				continue
			}
			// Cool-down only deprioritizes a fixed node; a sole WARP/proxy exit
			// must remain usable so a transient blip does not hard-block the scope.
	...
	if len(available) == 0 {
		// Configured fallback (fixed node / intentional direct) wins over reusing a
		// cooling primary. Operators opt into direct fallback explicitly.
		...
		if len(degraded) > 0 {
			// No fallback applied: reuse any still-enabled cooling fixed node so a
			// sole WARP/proxy exit is not hard-blocked by a transient blip.
			available = degraded
...
func (m *Manager) selectNode(nodes []domain.Node, affinity string) domain.Node {
	...
	for _, node := range nodes[1:] {
		...
		if current < bestCurrent || (current == bestCurrent && nodePreferable(node, best)) {
			best = node
			bestCurrent = current
		}
	}
...
func (m *Manager) isSoleEnabledNode(ctx context.Context, value domain.Node) bool {
	...
	// Only consult the hot snapshot. Feedback must never re-enter ListEgressNodes
	// while a load is blocked (deadlock with singleflight/repository stalls).
	// Without a snapshot, treat as multi-peer so transport failures still cool.
	// Sole reuse for single-exit setups still works at acquire time via degraded.
	_ = ctx
	m.nodeMu.RLock()
	snapshot, ok := m.nodes[value.Scope]
	m.nodeMu.RUnlock()
	if !ok {
		return false
	}
